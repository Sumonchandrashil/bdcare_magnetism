                ...
                <div class="modal-content">
                    ...
                    <form method="POST"  enctype="multipart/form-data" v-on:submit.prevent="store" class="m-form m-form--fit m-form--label-align-right">
                        <div class="modal-body">
                            <div class="form-group m-form__group row m--margin-top-20" :class="{'has-danger': (errors.hasOwnProperty('city_name'))}">
                                <label class="col-form-label col-lg-4 col-sm-12">City <span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input type="text" class="form-control" v-model="form.city_name" placeholder="Enter city name" />
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('city_name'))">{{ (errors.hasOwnProperty('city_name')) ? errors.city_name[0] :'' }}</div>
                                </div>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            store:function(){
                axios.post(base_url+'setup_city', this.form).then( (response) => {
                    $('#addModel').modal('hide');
            ...
            update:function (id) {
                axios.put(base_url+'setup_city/'+id, this.form).then( (response) => {
        ...
        mounted(){
           ...
           $('#addModel,#editModel').on('hidden.bs.modal', function(){
               _this.errors = {};
               _this.form = {'city_name':'','description':'','status':'1'};
           });

            EventBus.$on('update-button-clicked', (id) => {
                _this.errors = {};
                axios.get(base_url+'setup_city/'+id+'/edit').then((response) => {
