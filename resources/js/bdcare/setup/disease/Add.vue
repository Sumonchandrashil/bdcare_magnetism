            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="">Add Disease</h5>
                    ...
                    <form method="POST"  enctype="multipart/form-data" v-on:submit.prevent="store" class="m-form m-form--fit m-form--label-align-right">
                        <div class="modal-body">
                            <div class="form-group m-form__group row m--margin-top-20" :class="{'has-danger': (errors.hasOwnProperty('disease_name'))}">
                                <label class="col-form-label col-lg-4 col-sm-12">Disease <span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input type="text" class="form-control" v-model="form.disease_name" placeholder="Enter Disease name" />
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('disease_name'))">{{ (errors.hasOwnProperty('disease_name')) ? errors.disease_name[0] :'' }}</div>
                                </div>
                            </div>
                            <div class="form-group m-form__group row m--margin-top-20">
                                <label class="col-form-label col-lg-4 col-sm-12">Symptoms</label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <form method="POST" enctype="multipart/form-data" v-on:submit.prevent="update(form.id)" class="m-form m-form--fit m-form--label-align-right">
                        <div class="modal-body">
                            <div class="form-group m-form__group row m--margin-top-20" :class="{'has-danger': (errors.hasOwnProperty('disease_name'))}">
                                <label class="col-form-label col-lg-4 col-sm-12">Disease Name <span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input type="text" class="form-control" v-model="form.disease_name" placeholder="Enter Disease name" />
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('disease_name'))">{{ (errors.hasOwnProperty('disease_name')) ? errors.disease_name[0] :'' }}</div>
                                </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            store:function(){
                axios.post(base_url+'setup_disease', this.form).then( (response) => {
                    $('#addModel').modal('hide');
            ...
            update:function (id) {
                axios.put(base_url+'setup_disease/'+id, this.form).then( (response) => {
        ...
        mounted(){
           ...
           $('#addModel,#editModel').on('hidden.bs.modal', function(){
               _this.errors = {};
               _this.form = {'disease_name':'','description':'','status':'1'};
           });

            EventBus.$on('update-button-clicked', (id) => {
                _this.errors = {};
                axios.get(base_url+'setup_disease/'+id+'/edit').then((response) => {
